...
const MonthView: React.FC<Props> = ({ events, selectedDate, onEventClick, onDateSelect }) => {
  ...
  return (
    <div className="space-y-8">
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden p-2 md:p-3 max-w-[560px] mx-auto">
        ...
        <div className="grid grid-cols-7 gap-0.5 auto-rows-[28px] sm:auto-rows-[30px] md:auto-rows-[34px]">
            ...
            return (
              ...
              >
                ...
                {dayEvents.length > 3 && !isSelected && (
                  <span className="absolute right-1 bottom-0.5 hidden md:block text-[8px] font-black text-slate-400">+{dayEvents.length - 3}</span>
                )}
              </motion.div>
            );
          })}
        ...
        {daysWithEvents.length > 0 ? (
          <div className="space-y-8 max-w-[900px] mx-auto">
            {daysWithEvents.map((item) => (
              <div key={item.date.toISOString()} className="space-y-4">
                <div className="flex items-center gap-3 px-1">
                  <div className="h-2 w-2 rounded-full bg-blue-600 shadow-[0_0_10px_rgba(59,130,246,0.5)]" />
